...
.. note :: DOCUMENTATION INCOMPLETE. What does this do?
"""
import logging
...
# Why is GPsampler in regressors?
...
sampler = GPsampler(lower, upper, X_train, y_train, mean=meany,
                    n_stacks=n_GP_slices, add_train_data=True)
...
def display_results(regressors, qx, qy, grid_qry):
    ...
    plt.colormap = visvis.CM_JET
    a = ((regressors[0].X - np.array([np.min(qx),
         np.min(qy)])[np.newaxis, :]) /
         np.array([np.max(qx) - np.min(qx), np.max(qy) -
                  np.min(qy)])[np.newaxis, :]) * np.array(qx.shape)
    n = a.shape[0]
